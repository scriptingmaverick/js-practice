...
const totalOrbits = (rawMap) => {
  ...
  for (const key in orbits) {
    ...
    while (indirectOrbit !== undefined) {
      indirectOrbits++;
      console.log(orbit, indirectOrbit);
      orbit = indirectOrbit;
...
const minOrbitsToReachSanta = (rawMap) => {
  ...
  const maxDist = Math.max(
    ...(Object.values(orbitDistances).map((x) => x.length)),
  );

  // for (let i = 0; i < maxDist; i++) {
  // }
  // const sum = Object.values(orbitDistances).reduce((sum, orbit) => orbit + sum, 0);
  console.log(orbitDistances, maxDist);
  // console.log(sum);
};
